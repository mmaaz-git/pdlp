...
The code is meant to have a "linear flow" as much as possible, so that steps are "in line" as much as possible so the reader can see what is going on and easily map it to the math. The entire implementation is around 500 lines (and honestly, only 200 lines of real code, i.e., not printing or docstrings, etc.), entirely in one function `solve()` in `pdlp.py`.
